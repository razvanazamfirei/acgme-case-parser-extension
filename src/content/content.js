...
function getBlockadeSiteOptionsByType(areaLabel) {
  const optionsByType = new Map();
  const options = document.querySelectorAll(
    `input.cbprocedureid[data-area="${areaLabel}"]`,
  );

  for (const option of options) {
    const optionType = normalizeBlockadeSiteText(
...
function checkBlockadeSite(
  areaLabel,
  typeLabel,
  optionsByType = null,
  warningLabel = "blockade site",
  shouldWarn = true,
) {
  const targetType = normalizeBlockadeSiteText(typeLabel);
  const option =
    optionsByType?.get(targetType) ||
    getBlockadeSiteOptionsByType(areaLabel).get(targetType);

  if (!option) {
    if (shouldWarn) {
      console.warn(`${warningLabel} checkbox not found:`, typeLabel);
    }
    return false;
  }
...
function fillBlockadeSiteSelections({
  areaLabel,
  warningLabel,
  fieldKey,
  selectedTypes,
  ...
  const optionsByType = getBlockadeSiteOptionsByType(areaLabel);
  const unmatched = [];
  const filledTypes = new Set();

  for (const typeLabel of selectedTypes) {
    if (filledTypes.has(typeLabel)) {
      continue;
    }
    ...
    if (
      ...
      filledTypes.add(typeLabel);
      result.filled.push(`${fieldKey}:${typeLabel}`);
    } else {
      unmatched.push(typeLabel);
    }
  }

  if (unmatched.length > 0 && shouldWarn) {
    result.warnings.push(
      `Could not check ${warningLabel} for: ${unmatched.join(", ")}`,
    );
  }
...
function fillCase(caseData) {
  ...
  const neuraxialBlockadeSelection =
    resolveNeuraxialBlockadeSiteSelections(caseData);
  fillBlockadeSiteSelections({
    areaLabel: NEURAXIAL_BLOCKADE_SITE_AREA,
    warningLabel: "neuraxial block site",
    fieldKey: "neuraxialBlockadeSite",
    selectedTypes: neuraxialBlockadeSelection.selectedTypes,
    result,
    shouldWarn,
  });
  if (neuraxialBlockadeSelection.unmappedTokens.length > 0 && shouldWarn) {
    result.warnings.push(
      `Could not map neuraxial block site for: ${neuraxialBlockadeSelection.unmappedTokens.join(", ")}`,
    );
  }

  const peripheralBlockadeSelection =
    resolvePeripheralBlockadeSiteSelections(caseData);
  fillBlockadeSiteSelections({
    areaLabel: PERIPHERAL_NERVE_BLOCKADE_SITE_AREA,
    warningLabel: "peripheral block site",
    fieldKey: "peripheralBlock",
    selectedTypes: peripheralBlockadeSelection.selectedTypes,
    result,
    shouldWarn,
  });
  if (peripheralBlockadeSelection.mappedToOther.length > 0 && shouldWarn) {
    result.warnings.push(
      `Mapped peripheral block site to "${OTHER_PERIPHERAL_NERVE_BLOCKADE_SITE}" for: ${peripheralBlockadeSelection.mappedToOther.join(", ")}`,
    );
  }
